...
public class Count_crossing_number {
	...
	int[][] AgglomerativeBundling_edges(int n, int m,int [][] graph ,int [][] Edges_set, vertice3D[] vertice_arr, int [][] ordering)
	{
		...
		*/
		...
		for( i=0 ; i<m ; i++)
		{
			if( graph_edges_numbers[0][i] == 1 && graph_edges_numbers[1][i] == 51 ) 
			{
				for( j=0 ; j<m ; j++)
				{
					if( j == i) 
			...
			if( graph_edges_numbers[0][i] == 4 && graph_edges_numbers[1][i] == 31 ) 
			{
				for( j=0 ; j<m ; j++)
				{
					if( j == i) 
			...
			if( graph_edges_numbers[0][i] == 55 && graph_edges_numbers[1][i] == 91 ) 
			{
				for( j=0 ; j<m ; j++)
				{
					if( j == i) 
			...
			if( graph_edges_numbers[0][i] == 64 && graph_edges_numbers[1][i] == 72 ) 
			{
				for( j=0 ; j<m ; j++)
				{
					if( j == i) 
		...
		for( j = 0 ; j < m ; j++ )
		{
			if( arr_A[j] == 1) 
			{
				...
				System.out.printf("arr_A[" + j + "] = " + arr_A[j] + "\n");
			}
			if( arr_B[j] == 1)
			{
				System.out.printf("j = " + j + "\n");
				System.out.printf("arr_B[" + j + "] = " + arr_B[j] + "\n");
			}
		}
		...
		for( i=0 ; i<m ; i++ )
		{
			if( arr_A[i] == 1)
			{
				for( j=0 ; j<m ; j++ )
				{
					if( arr_B[j] == 1)
					{
						if(ordering[0][i] == ordering[0][j] || ordering[1][i] == ordering[1][j])
						{
							return 1;
						}						
	...
	double Algo_bundling_edge_length(int m,int n, int [] arr_A , vertice3D [] vertice_arr , int [][] graph_edges_numbers)
	{
		...
		//System.out.printf(length(centroid_x1 - centroid_x2, centroid_y1 - centroid_y2) + "\n");
		//System.out.printf(total_length + "\n");
...
/*
try {
    PrintStream out = new PrintStream(new FileOutputStream("C:/Users/firestation/workspace/Graph drawing/src/Edge_bundling/graph_edges_numbers.txt"));
    for( i=0 ; i<m ; i++) 
    	out.println("graph_edges_numbers[ " + 0 + "][" + i +  "] = " + graph_edges_numbers[0][i] + "     graph_edges_numbers[ " + 1 + "][" + i +  "] = " + graph_edges_numbers[1][i] +"\n");
    out.close();
